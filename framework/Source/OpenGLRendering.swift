...
public func renderQuadWithShader(_ shader:ShaderProgram, uniformSettings:ShaderUniformSettings? = nil, vertices:[GLfloat]? = nil, vertexBufferObject:GLuint? = nil, inputTextures:[InputTextureProperties], calVertexNum:Bool = false) {
    ...
    if calVertexNum, let boundVBO = vertexBufferObject {
        var size:GLint = 0
        glBindBuffer(GLenum(GL_ARRAY_BUFFER), boundVBO)
        glGetBufferParameteriv(GLenum(GL_ARRAY_BUFFER), GLenum(GL_BUFFER_SIZE), &size);
        size = size / Int32(MemoryLayout<GLfloat>.size) / 2
        glDrawArrays(GLenum(GL_TRIANGLE_STRIP), 0, size)
    } else {
